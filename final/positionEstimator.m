function [x, y, modelParameters] = positionEstimator(past_current_trial, modelParameters)
    ...
    test_time_bin = (current_trial_length/bin_size) - (start_time/bin_size)+1;
         
    training_max_timebin = modelParameters.max_timebin_index; % max time bin from training dataset
    training_largest_timebin = training_max_timebin - (start_time/bin_size); % largest index for pcr parameters gotten from training
    training_spike_length = training_max_timebin*bin_size; % limit of spike length used in training
