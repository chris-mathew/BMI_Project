...
function [X, Y] = prepareSVMTrainingData(training_data)
    ...
    for single_trial = 1:numTrials
        for direction = 1:numDirections
            ...
            avgSpikes = mean(spikes(:, 1:initialTimeWindow), 2)'; % Row vector of average spike counts (1x98 mean across timesteps, so mean spikes for each neuron)
            X = [X; avgSpikes]; % Append to feature matrix (100 x 98)
...
function model = trainSVM(X, Y, kernelType)
    ...
    m = size(X, 1); % Number of training examples = 400
    n = size(X, 2); % Number of features = 98
